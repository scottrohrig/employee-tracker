...
const options = [
    ...
    "View employees assigned to a given manager",
    "View all employees groups by manager",
    "View Department Budget Allocation",
    // TODO:
    //      update employee manager
    //      view employees grouped be department
    //      delete roles
    //      delete employees
    //      view total budget of a given department (combined salaries)
...
const viewEmployeesByManager = async () => {
    ...
    let { manager } = await inquirer.prompt( {
        type: 'list',
        name: 'manager',
        message: 'Select a manager to view his/her staff',
        choices: managers.map( ( { first_name, last_name, id } ) => `${ id }:${ first_name } ${ last_name }` )
    } );

    let manager_id = manager.split( ':' )[ 0 ];
...
const viewEmployeesGroupedByManager = async () => {
...
const viewDepartmentBudget = async () => {
    // NOT QUITE FINISHED. Needs to aggregate employees, not just roles.
    const sql = `
        SELECT 
            department.name, 
            sum(role.salary) as budget_actual 
        FROM role 
...
const start = async () => {
    ...
    switch ( option ) {
        ...
        case options[ 7 ]:
            ...
            break;
        case options[ 8 ]:
            const staff = await viewEmployeesByManager();
            console.table( staff );
            start();
            break;
        case options[ 9 ]:
            const groupedByManager = await viewEmployeesGroupedByManager();
            console.table( groupedByManager );
            start();
            break;
        case options[ 10 ]:
            const budgetActual = await viewDepartmentBudget();
            console.table( budgetActual );
            start();
            break;
        case 'Exit':
            process.exit();
